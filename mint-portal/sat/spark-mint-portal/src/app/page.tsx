...
const T = {
  ENG: {
    ...
    intro: 'Enter your sp1 → Generate → send the required sats → paste Tx Hash → Verify. Tokens will be minted to the payer address.',
    ...
    fcfsClaimed: 'You already claimed the Free Mint. You can continue with Paid Mint for more tokens.',
    copied: 'Copied',
    copyFailed: 'Copy failed',
    holderTitle: 'Holder Claim',
    holderIntro: 'If you hold eligible token(s), claim here (1x per address). No payment required.',
  ...
  ZH: {
    title: '铸造 BTKN（Spark）',
    intro: '输入你的 sp1 → 生成 → 按所需 sats 支付 → 粘贴交易哈希 → 验证。代币将铸造到付款地址。',
    ...
    tooEarly: '过早请求，正在冷却…',
    fcfsClaimed: '你已领取免费铸造。可继续进行付费铸造获取更多代币。',
    copied: '已复制',
    copyFailed: '复制失败',
    holderTitle: '持有者领取',
    holderIntro: '如果你持有符合条件的代币，可在此领取（每个地址限 1 次）。无需支付。',
    holderInput: 'A) 持有者地址',
    holderCheck: '检查并准备领取',
    holderStatus: '状态',
    holderEligible: '符合持有者资格',
    holderNotEligible: '不符合持有者资格',
    holderClaimed: '已领取',
...
function baseToTokenStr(base: string | number | bigint){
  try{
    ...
    const pow = 10n ** d;
    const whole = n / pow;
    const frac = (n % pow).toString().padStart(TOKEN_DEC, '0').replace(/0+$/, '');
...
function summarizeError(j: any): string {
  ...
  if (e === 'rate_limited') {
    ...
    if (j.retryAfterMs) lines.push(`Retry after ~${Math.ceil(Number(j.retryAfterMs)/1000)}s`);
  } else if (e === 'too_early') {
    ...
    lines.push('OG: not eligible (need tx before cutoff).');
    if (j.cutoffMs) lines.push(`Cutoff: ${new Date(Number(j.cutoffMs)).toISOString()}`);
  } else if (e === 'og_tx_invalid') {
    lines.push('OG: tx invalid / not parseable for sender.');
  } else if (e === 'holder_already_claimed') {
    lines.push('Holder: already claimed.');
  } else if (e === 'fcfs_sold_out') {
    lines.push('FCFS: sold out.');
  } else if (e === 'verify_failed' || e === 'bad_json') {
    lines.push('Explorer error / bad response (maybe API rate limit).');
  } else {
    lines.push(`Error: ${e}`);
  }
  return lines.join('\n');
...
function summarizeQuote(j: any): string {
  ...
  const lines: string[] = [];
  if (j.suggestedTier) lines.push(`Tier: ${j.suggestedTier}${j.tierLabel ? ` (${j.tierLabel})` : ''}`);
  if (j.fcfs) {
    lines.push(
  ...
  if (j.paid) {
    const tier = j.paid.cohortIndex ? `Tier ${j.paid.cohortIndex}/${j.paid.totalCohorts}` : '—';
    const price = j.paid.priceSats ? fmtSats(j.paid.priceSats) : '';
    lines.push(`Paid: ${tier}${price ? ` • ${price}` : ''} • minted ${j.paid.mintedCount}`);
  }
  if (j.feeAddress) lines.push(`Pay to: ${j.feeAddress}`);
  if (j.requiredAmountSats || j.amount) lines.push(`Amount: ${fmtSats(j.requiredAmountSats || j.amount)}`);
  return lines.join('\n');
}
...
function summarizeVerify(j: any): string {
  ...
  if (j.minted === true) {
    ...
    if (j.cohort?.index) lines.push(`Cohort: Tier ${j.cohort.index}${j.cohort.slot ? ` • Slot ${j.cohort.slot}` : ''}`);
...
function summarizeHolder(j: any): string {
  ...
  lines.push(`Status: ${j.claimed ? 'Already claimed' : (j.eligible ? 'Eligible' : 'Not eligible')}`);
  if (j.matchedIds?.length) lines.push(`IDs: ${j.matchedIds.join(', ')}`);
  if (j.matchedTickers?.length) lines.push(`Tickers: ${j.matchedTickers.join(', ')}`);
  if (!j.matchedIds?.length && j.tokenIds?.length) lines.push(`IDs: ${j.tokenIds.join(', ')}`); 
  return lines.join('\n');
}
...
export default function Page(){
  ...
  async function requestQuote(){
    setLog('');
    if (inCooldown) { setLog('Cooling down…'); return; }
    if(!looksLikeSpark(receiver)){
      setLog('Enter a valid sp1…');
      return;
    }
    ...
    try {
      ...
      try{
        ...
        holderLine = hr.ok ? `Holder: ${hj.claimed ? 'already claimed' : (hj.eligible ? 'eligible' : 'not eligible')}` : summarizeError(hj);
      }catch{}
      ...
      setQuote(null);
      setLog(summarizeError({ error:'network_error', detail: String(e?.message||e) }));
    }
  }
  ...
  useEffect(()=>{ (async()=>{
    ...
    try{
      const uri = bip21(quote.feeAddress, amt);
  ...
  async function verifyAndSend(){
    setLog('');
    if (inCooldown) { setLog('Cooling down…'); return; }
    if(!quote?.orderToken){ setLog('Order token is not ready. Click Generate first.'); return; }
    if(!txId.trim()){ setLog('Tx hash is required'); return; }
    setVerifying(true);
  ...
  async function checkHolder(){
    setHolderLog('');
    setHolderInfo(null);
    if (!looksLikeSpark(holderAddr)) {
      setHolderLog('Enter a valid sp1…');
      return;
    }
    setHolderBusy(true);
    try{
      const r = await fetch(apiPath('/api/holder/request'), {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ receiverSparkAddress: holderAddr }),
        cache: 'no-store'
      });
      const j = await safeJsonResponse(r);
      ...
      setHolderLog(summarizeError({ error:'network_error', detail: String(e?.message||e) }));
    }
    setHolderBusy(false);
  }
  ...
  const holderStatusText =
    holderInfo?.claimed ? T[lang].holderClaimed :
    holderInfo?.eligible ? T[lang].holderEligible : T[lang].holderNotEligible;
  ...
  return (
    <main className="space-y-6">
      ...
      {inCooldown && <div className="card text-sm">{t.cooldownMsg} ({cooldownLeftSec}s)</div>}
        ...
        <section className="card space-y-3">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <div className="mt-2 text-sm">{t.requiredAmount}</div>
              <div className="text-xs">
                <code>{shownAmount}</code> sats
                <span className="opacity-70"> (~{satsToBtcStr(shownAmount)} BTC)</span>
              </div>

              <div className="mt-2 text-sm">{t.detectedTier}</div>
              <div className="text-xs">{tierLine}</div>

              {quote.fcfs && (
                <div className="mt-2 text-xs opacity-80">
                  FCFS: price {quote.fcfs.priceSats} sats • {quote.fcfs.taken}/{quote.fcfs.limit} claimed{quote.fcfs.alreadyClaimed ? ' • already claimed' : ''}
                </div>
              )}
              {quote.paid && (
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                <button className="btn" onClick={()=>copy(quote.feeAddress)} disabled={inCooldown}>{t.copyAddress}</button>
                <button className="btn" onClick={()=>copy(shownAmount)} disabled={inCooldown}>{t.copyAmount}</button>
                <button className="btn" onClick={()=>window.open('https://www.sparkscan.io/address/' + quote.feeAddress,'_blank')}>{t.openSparkScan}</button>
                <button className="btn" onClick={()=>window.open(bip21(quote.feeAddress, shownAmount),'_blank')}>{t.openInWallet}</button>
              </div>
            </div>
      ...
      {quote && (
        <section className="card space-y-3">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div>
              <label className="label">{t.txHash}</label>
              <input className="input" placeholder={t.txPlaceholder} value={txId} onChange={e => setTxId(e.target.value)} disabled={inCooldown} />
              <p className="text-xs text-neutral-400 mt-1">{t.required}</p>
            </div>
            <div>
              <label className="label">{t.payerOptional}</label>
              <input className="input" placeholder={t.payerPlaceholder} value={payer} onChange={e => setPayer(e.target.value)} disabled={inCooldown} />
            </div>
          </div>
        </section>
      )}
      ...
      {quote && (
        <section className="card space-y-3">
          ...
          <button className="btn" onClick={verifyAndSend} disabled={!canVerify}>
            {inCooldown ? T[lang].cooldownBtn(cooldownLeftSec) : (verifying ? t.verifying : t.verifyBtn)}
          </button>
      ...
      <section className="card space-y-3">
        ...
        <input className="input" placeholder="sp1…" value={holderAddr} onChange={e=>setHolderAddr(e.target.value)} />
        <div className="flex gap-2">
          <button className="btn" onClick={checkHolder} disabled={holderBusy}>{t.holderCheck}</button>
          <button
            className="btn"
            onClick={claimHolder}
            disabled={holderBusy || !holderInfo?.ok || !holderInfo.eligible || holderInfo.claimed}
          >
            {t.holderBtn}
          </button>
        </div>
        ...
        {holderInfo && (
          <div className="mt-3 space-y-1 text-sm">
            <div>Address: <span className="font-mono break-all">{holderInfo.receiver}</span></div>
            <div>{t.holderStatus}: {holderStatusText}</div>
