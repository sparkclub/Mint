...
export async function upsertEnv(pairs: Record<string,string>, filePath = '.env.local') {
  ...
  if (process.env.VERCEL) return { ok:false as const, reason:'read_only_env', file:filePath };
  ...
  return { ok:true as const, file: path.resolve(filePath) };
}

export const upsertEnvVars = upsertEnv;

export async function upsertEnvFile(pairs: Record<string,string>, filePath = '.env.local'): Promise<string> {
  const res = await upsertEnv(pairs, filePath);
  return res.file;
}
